...
# ====== LLM（生成端，預設用 OpenAI；可自行替換成 Cohere 或其他） ======
...
def init_braintrust_experiment(project: str, experiment: str):
    ...
    if not api_key:
        print("⚠️  未偵測到 BRAINTRUST_API_KEY，將以離線模式執行（不會上傳到 Braintrust）。")
        return None
    exp = bt.init(
        project=project,
...
def evaluate_one(sample: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # RAGAS
    rag = ragas_scores(sample, raw_text)
...
def main():
    ...
    for s in SAMPLES:
        ...
        # === 終端列印（離線或輔助觀察） ===
        sc = r["scores"]
        print(f"{r['sid']}: final={sc['final_score']:.3f} faith={sc['faithfulness']:.2f} "
              f"LLM(reason)={sc['reason_llm']:.2f} citeP={sc['citation_precision']:.2f} "
              f"format={sc['format_ok']} correct={sc['correctness_ok']}")

    if exp is not None:
        bt.flush()  # 送出批次事件
        print(f"✅ 已上傳到 Braintrust 專案：{project} / 實驗：{experiment}")
