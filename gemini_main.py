...
# ====== Gemini：扎根評估（替代 RAGAS） ======
# 使用 Gemini 進行 RAG 評估，避免需要 OpenAI API Key
...
# ---------------------------
# 4) Gemini：扎根評估（替代 RAGAS）
# ---------------------------
def gemini_rag_scores(sample: Dict[str, Any], model_text: str) -> Dict[str, float]:
    """使用 Gemini 進行 RAG 評估，替代 RAGAS"""
    contexts = [sample["question"], sample["explanation"]] + list(sample["options"].values())
    
    # 評估忠實度：AI 回饋是否基於提供的上下文
    faithfulness_prompt = f"""你是一位嚴格的評估專家。評估以下教師回饋是否完全基於提供的上下文，不添加外部知識。
...
請評估回饋的忠實度（0-1分），只基於上下文內容，不添加外部知識。
回傳 JSON 格式：{{"score": <0-1>, "explanation": "..."}}"""

    # 評估上下文精確度：回饋是否準確引用上下文
    precision_prompt = f"""評估以下教師回饋是否準確引用和使用了提供的上下文。
...
請評估上下文精確度（0-1分），回饋是否準確使用了上下文中的關鍵信息。
回傳 JSON 格式：{{"score": <0-1>, "explanation": "..."}}"""

    # 評估答案相關性：回饋是否與問題相關
    relevancy_prompt = f"""評估以下教師回饋是否與學生問題高度相關。

問題：{sample["question"]}
學生作答：{sample["student_answer"]}
正確答案：{sample["answer"]}

教師回饋：
{model_text}

請評估答案相關性（0-1分），回饋是否針對性地回應了學生的具體問題和作答。
回傳 JSON 格式：{{"score": <0-1>, "explanation": "..."}}"""

    def _call_gemini_for_score(prompt: str) -> float:
        if not genai_client:
            print("[WARN] 未偵測到 GEMINI_API_KEY，使用預設分數")
            return 0.5  # 預設中等分數
        ...
        try:
            ...
            print(f"[DEBUG] Gemini 原始回應: {text[:200]}...")
            
            # 解析 JSON 分數
            try:
                j = json.loads(re.search(r"\{[\s\S]*\}", text).group(0))
                score = float(j.get("score", 0.0))
                print(f"[DEBUG] 成功解析 JSON 分數: {score}")
            except Exception as e:
                print(f"[DEBUG] JSON 解析失敗: {e}")
                m = re.search(r"0?\.\d+|1(?:\.0+)?", text)
                score = float(m.group(0)) if m else 0.0
                print(f"[DEBUG] 正則提取分數: {score}")
            return max(0.0, min(1.0, score))
        except Exception as e:
            print(f"[DEBUG] Gemini API 呼叫失敗: {e}")
            return 0.5  # 預設中等分數

    # 執行三個評估
    faithfulness_score = _call_gemini_for_score(faithfulness_prompt)
    precision_score = _call_gemini_for_score(precision_prompt)
    relevancy_score = _call_gemini_for_score(relevancy_prompt)
    
    print(f"[DEBUG] Gemini RAG 評估結果:")
    print(f"  - 忠實度: {faithfulness_score:.3f}")
    print(f"  - 上下文精確度: {precision_score:.3f}")
    print(f"  - 答案相關性: {relevancy_score:.3f}")
    
    return {
        "faithfulness": faithfulness_score,
        "context_precision": precision_score,
        "context_recall": precision_score,  # 簡化，用精確度代替
...
def evaluate_one(sample: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Gemini RAG 評估（替代 RAGAS）
    rag = gemini_rag_scores(sample, raw_text)
